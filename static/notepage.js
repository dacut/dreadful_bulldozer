jQuery(function($) {
    ...
    function onMouseMove(e) {
        // Handle a mouse motion event on the canvas.  If a drag event is in
        // progress, this updates the position of the note on the canvas.
        var deltaX, deltaY, left, top;

        if (dragNoteDOM) {
            // A note is being moved.

            if (e.which !== 1) {
                // Mouse button was released, but we didn't receive the
                // mouseup event.  Go ahead and release the drag now.
                stopDragging();
                return;
            }
